...
LicenseData parseInput(string& message)
{
	...
	while(!input.eof())
	{
		...
		if (std::string::npos != delimiterPos) {
			...
			} else if (name == LICENSE_DATE) {
				result.purchaseDate = value;
			} else if (name == LICENSE_PRODUCT) {
				result.productId = value;
			} else if (name == LICENSE_QUANTITY) {
				result.quantity = value;
			} else if (name == LICENSE_VERSION) {
...
bool IsLicensed()
{
	////////////////////////////////////////////////
	// Setup
	string message = "TElDRU5TRV9JRD0wOjEKTElDRU5TRV9EQVRFPTA0LzA3LzIwMDQKTElDRU5TRV9QUk9EVUNU\
...
T2+XMh3nEfmzqQ==";
	...
	LicenseData data = parseInput(recovered);
